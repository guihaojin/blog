...
使用过C++/Java这些编译型语言跟php, python, ruby这些脚本语言完成过一些基本的文本处理之类的任务人不难发现脚本语言的方便之处。概括起来主要有2点：

1. 编译跟执行在一起，改完了直接run，不用再重新编译；
2. 没有麻烦的入口函数/main函数之类的东西，一行行执行，简单明了；
...
我觉得以上的东西对Ruby而言很重要，但是都是不是Ruby真正让人尖叫的地方。Ruby真正让我刮目相看的是它的**开放性**，**程序块**(block)以及其它对**函数式编程**的有力支持的特性。
